...
module.exports = (err, events, config) => `
  <div class="gh-events-widget ${escape(config.skin)}">
    <div class="header">
    ...
    Activity of <a href="https://github.com/${escape(config.user)}">@${escape(config.user)}</a></div>
    ${events.map(event => `
      ...
      `)
    .join('')}
    ${err ? `<div class="error">${escape(err.message)}</div>` : ''}
